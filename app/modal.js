...
export default function Modal() {
    ...
    return ReactDom.createPortal(
        <div className="fixed top-0 left-0 w-screen h-screen z-50">
            ...
            <div className="flex flex-col bg-white absolute right-0 top-0 h-screen shadow-lg w-screen gap-4 sm:w-96 max-w-screen">
                ...
                <div className="p-4 overflow-scroll flex-1 flex flex-col gap-4">
                    {cartItems.length === 0 ? (
                        <p>There is nothing on your Card</p>
                    ) : (
                        <>
                        {cartItems.map((cartItem, itemIndex) => {
                            return (
                                <div key={itemIndex} className="flex border-l border-solid border-slate-700 px-2 flex-col gap-2">
                                    ...
                                    <h2>{cartItem.name}</h2>
                                    <p>Rp. {cartItem.cost / 100 }</p>
                                    </div>
                                    <p className="text-slate-600 text-sm">Quantity: {cartItem.quantity} </p>
                                </div>
                            )
                        })}</>
                    )}
